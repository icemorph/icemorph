...
sub forward_viterbi
{
  my ($self, $observation, $restrictions_pf, $restrictions_ed) = @_;
...
#print "\t\tstate = $state\n";
	...
	$total += $prob;
        ...
        my $ir = 0;
        my $restricted = 0;
        if ( $restrictions_pf->{$output} ) {
           $restricted = 1;
           foreach my $pos ( @{$restrictions_pf->{$output}} ) {
              if ( $pos eq $state ) {
                 $ir = 1;
#print "\t\t\trestricted (pf)\n";
              }
        ...
        if ( ! $restricted && $restrictions_ed->{$output} ) {
           foreach my $pos ( @{$restrictions_ed->{$output}} ) {
              if ( $pos eq $state ) {
#print "\t\t\trestricted (ed)\n";
                 $ir = 1;
              }
        ...
        if ( $ir ) {
           if ( ! $is_restricted || $v_prob > $valmax ) {
              $is_restricted = 1;
	      $argmax = [ @$v_path, $next_state ];
	      $valmax = $v_prob;
#print "\t\t\tnew val: $valmax\n";
           }
        } else {
	   if ( ! $is_restricted && $v_prob > $valmax) {
	      $argmax = [ @$v_path, $next_state ];
	      $valmax = $v_prob;
#print "\t\t\tnew val2: $valmax\n";
	   }
        }
      }
